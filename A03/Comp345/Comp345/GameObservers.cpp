...
void GameStateView::Display() {
	...
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE); // change colors
	int maxLength = max(game->maxPlayerNameLength + 5, 16); // 5 for padding, 16 default values
...
{
	Player* currentPlayer = game->players[game->currentPlayer]; // current player
	if(game->getCurrentTurn() != 0) {
		cout << "+==============================+" << endl;
		cout << "+ ROUND #" << game->getCurrentTurn() << ": It's player's " << currentPlayer->GetLastName() << " turn +" << endl;
		cout << "+==============================+" << endl;
		cout << "Selects card number " << currentPlayer->getHand()[currentPlayer->getHand().size()-1];
